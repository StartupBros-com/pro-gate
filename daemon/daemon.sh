...
SYNCHRONOUS EXECUTION (critical): you are running headless: you will NOT receive any asynchronous background-task notification, so you must block yourself in a loop. After you launch the oracle review (background, no caller-side timeout on the launch itself), run: \"\${PRO_GATE_HOME:-\$HOME/.pro-review-daemon}/oracle-review.sh\" --wait '<out>' --timeout 1500 (repeatedly; each call blocks up to 25 minutes, safely under your own tool-call cap). Exit 20 means it timed out while the run stayed healthy: re-arm by calling the exact same --wait command again immediately, no changes. Exit 21 means lost observability (the status has not advanced within its bound): do NOT try to work around this yourself and do NOT post a PR comment — stop and end your turn reporting exactly that ('--wait exited 21: lost observability for <out>'); the daemon reads the engine's own state after your turn ends and decides whether to defer or fail this cycle, so guessing here only hides a real signal. Exit 0 means a terminal phase was reached: read the printed status JSON's 'phase' field. Phase 'done' means read the --out file; 'failed'/'deferred'/'oversized'/'round-capped' are terminal: report them, do NOT relaunch ('round-capped' means this PR already spent its Pro review round budget for the window: post ONE short PR comment saying the pro-gate round budget is exhausted and a human should decide whether to force another round, QUOTE the status file's 'detail' field in it, and if that detail reports an unconfirmed OPEN P0 lead the comment with that fact, then stop). Phase 'in-progress' means the model is STILL generating after the engine's budget: do NOT relaunch; wait 10 minutes, read the marker field from the status JSON, then run the deployed engine again as: \"\${PRO_GATE_HOME:-\$HOME/.pro-review-daemon}/oracle-review.sh\" --harvest '<marker>' --out '<out>' --timeout 20m (repeat while it exits 9; it spends no new quota). The oracle takes 10-30 minutes; that is expected. Do NOT end your turn, and do NOT say 'I will be notified', while the oracle is still running. Your turn is only complete once the PR comment has actually been posted.
