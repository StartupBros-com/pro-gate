...
SYNCHRONOUS EXECUTION (critical): you are headless and receive NO asynchronous notification, so block yourself. Before launch, capture a PRIOR_MARKER only from a regular non-symlink <out>.status (use jq when installed, otherwise sed; never follow a sidecar symlink). Launch in the background with NO caller-side timeout and assign ENGINE_PID=\$! BEFORE disown. Poll once per second in a loop that reads the regular non-symlink sidecar BEFORE each ENGINE_PID liveness check, so a fast terminal run gets one final read after exit. Accept MARKER only when it is nonempty, starts pg-run-, its final '-' component equals ENGINE_PID, and it differs from PRIOR_MARKER. If ENGINE_PID exits without one, print the final regular sidecar when present and stop — never loop on a stale sidecar. Then run: \"\${PRO_GATE_HOME:-\$HOME/.pro-review-daemon}/oracle-review.sh\" --wait \"\$MARKER\" --timeout 1800 repeatedly (each chunk is <=30 minutes). Exit 20 means it timed out while the run stayed healthy: re-arm by calling the exact same --wait command again immediately, no changes. Exit 21 means lost observability (the status has not advanced within its bound): do NOT try to work around this yourself and do NOT post a PR comment — stop and end your turn reporting exactly that ('--wait exited 21: lost observability for <marker>'); the daemon reads the engine's own state after your turn ends and decides whether to defer or fail this cycle, so guessing here only hides a real signal. Exit 0 means a terminal phase was reached: read the printed status JSON's 'phase' field. Phase 'done' means read the --out file; 'failed'/'deferred'/'oversized'/'round-capped' are terminal: report them, do NOT relaunch ('round-capped' means this PR already spent its Pro review round budget for the window: post ONE short PR comment saying the pro-gate round budget is exhausted and a human should decide whether to force another round, QUOTE the status file's 'detail' field in it, and if that detail reports an unconfirmed OPEN P0 lead the comment with that fact, then stop). Phase 'in-progress' means the model is STILL generating after the engine's budget: do NOT relaunch; wait 10 minutes, read the marker field from the status JSON, then run the deployed engine again as: \"\${PRO_GATE_HOME:-\$HOME/.pro-review-daemon}/oracle-review.sh\" --harvest '<marker>' --out '<out>' --timeout 20m (repeat while it exits 9; it spends no new quota). The oracle takes 10-30 minutes; that is expected. Do NOT end your turn, and do NOT say 'I will be notified', while the oracle is still running. Your turn is only complete once the PR comment has actually been posted.
