...
active plugin to the runtime's version instead, and only run the downgrade if you truly mean it. If
updating the plugin won't reconcile the versions, fully quit and relaunch Claude Code (a mid-session
reload can keep the old plugin cache bound), then re-run the doctor; a persistent mismatch means the
plugin and runtime came from different update paths — align them via a marketplace reinstall or a
matching runtime install.
