...
# pg_oracle_prompt_provably_unsubmitted <slug> <run-marker>: true only when Oracle's stored,
# machine-readable session metadata carries the complete 0.17+ prompt-commit-timeout proof that
# no ChatGPT turn landed. Generic lifecycle log lines are weaker: Oracle emits them before commit
# verification. Any missing, malformed, contradictory, foreign-marker, or unfamiliar shape fails
# closed so callers retain the round and suppress a potentially duplicate submission.
pg_oracle_prompt_provably_unsubmitted() {
  local slug="${1:-}" marker="${2:-}" oracle_home meta
  case "$slug" in ''|*[!A-Za-z0-9._-]*) return 1 ;; esac
  pg_reservation_marker_ok "$marker" || return 1
  pg_have jq || return 1

  oracle_home="${ORACLE_HOME_DIR:-${HOME:-}/.oracle}"
  [ -n "$oracle_home" ] || return 1
  meta="$oracle_home/sessions/$slug/meta.json"
  [ -f "$meta" ] && [ ! -L "$meta" ] || return 1

  jq -e --arg slug "$slug" --arg marker "$marker" '
    . as $meta
    | ($meta.error.details.commitProbe // null) as $probe
    | ($meta.id == $slug)
      and ($meta.status == "error")
      and ($meta.mode == "browser")
      and ($meta.error.category == "browser-automation")
      and ($meta.error.details.stage == "submit-prompt")
      and ($meta.error.details.code == "prompt-commit-timeout")
      and (($meta.error.details.promptLength | type) == "number")
      and ($meta.error.details.promptLength > 0)
      and (($meta.error.details.timeoutMs | type) == "number")
      and ($meta.error.details.timeoutMs > 0)
      and (($meta.options.prompt? | strings | contains("run marker: \($marker) ")) // false)
      and ($meta.browser.runtime.promptSubmitted == true)
      and ($meta.browser.runtime.tabUrl == "https://chatgpt.com/")
      and (($probe | type) == "object")
      and ($probe.baseline == 0)
      and ($probe.turnsCount == 0)
      and ($probe.userMatched == false)
      and ($probe.prefixMatched == false)
      and ($probe.lastMatched == false)
      and ($probe.hasNewTurn == false)
      and ($probe.stopVisible == false)
      and ($probe.assistantVisible == false)
      and ($probe.composerCleared == true)
      and ($probe.inConversation == false)
      and ($probe.editorLength == 0)
      and ($probe.lastTurnLength == 0)
  ' "$meta" >/dev/null 2>&1
}
