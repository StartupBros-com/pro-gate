...
# Remote use:
#   curl -fsSL "https://raw.githubusercontent.com/StartupBros-com/pro-gate/v0.23.1/install.sh?$(date +%s)" | bash -s -- --version 0.23.1
set -euo pipefail
umask 022
...
# install that left keys group/other-readable is worse than failing loudly.
chmod 600 "$PRO_GATE_HOME/.env" \
  || { echo "ERROR: could not secure $PRO_GATE_HOME/.env (chmod 600 failed) — it may hold keys; fix ownership/permissions and re-run" >&2; exit 1; }
