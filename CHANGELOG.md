...
|---|---|---|---|
| v0.52.0 | 2026-09-17 | Pending release | Operational hardening round: a superseded round's ChatGPT tab is closed on `--recover` instead of leaking one tab per round (#206); a stale rate-limit modal on an unowned tab no longer re-arms the account cooldown on every attempt (#208); the account-slot wait (#179) and `pg_lock`'s no-flock reclaim spin (#189) are bounded; a selection file with one trailing newline is accepted (#203); a connector-only classic run installs a connector binding, never a full-pr proof (#150); `install.sh` hashes through stdin (#172); remaining bare empty-array expansions guarded for bash 3.2 (#176) |
