...
|---|---|---|---|
| v0.51.0 | 2026-09-14 | Pending release | An attempt for which no conversation was ever observed is its own terminal kind, `never-conversed`: bounded confirmed absences release its shared slot without the 6h TTL, gated on a durable stamp so an attempt predating sighting-recording is never retired on a false absence, and every page read records its sighting at the single text producer so no caller can exit ahead of it |
