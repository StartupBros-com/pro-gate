...
1. **Enforce the exact plugin runtime.** Resolve this plugin's promoted version and run the
   ...
   the operator to update the active plugin instead unless the downgrade is intended (and if updating
   won't reconcile them, to fully relaunch Claude Code and re-run the doctor before reinstalling).
