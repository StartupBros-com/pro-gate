...
- **Invoke a review:** `oracle -e browser --remote-chrome 127.0.0.1:9222 -m gpt-5.6 -p "..." --write-output <file>` (targets the GPT-5.6 Sol flagship; see `.env.example` for `select` force+verify vs `current`). Historical proof (2026-06-29, GPT-5.5 era, preserved verbatim): `requested=Pro; resolved=Pro Extended; verified=yes`, i.e. `-m gpt-5.5-pro` landed on the Pro thinking tier. ~18s for a trivial prompt; real reviews ~10min (oracle `--timeout auto`=60m for Pro; long runs detach → reattach with `oracle session <slug>`, never re-run). NOTE (oracle 0.16.0): use `current` and set the account picker to **"Pro"** (= GPT-5.6 Sol Pro, the top tier) via `login-view.sh`. Do NOT use `select` with `gpt-5.6`: oracle expands it to the label "GPT-5.6 Sol" and fails to match it in the model switcher (offered: Instant5.5, Medium, High, Extra High, Pro, GPT-5.6 Sol), so the run fails model selection without submitting (dogfood 2026-07-17, PR #32). The engine now auto-falls-back `select`->`current` on that error.
